            ...
            <div class="modal-content">
                ...
                <form action="" id="m_form" method="POST"
                    ...
                    <div class="modal-body">
                        ...
                        <label for="slug">{{ __('voyager::menu_builder.url_slug') }}</label>
                        <input type="text" class="form-control" id="m_slug" name="slug"
                               placeholder="{{ __('voyager::generic.url_slug') }}"><br>
                        ...
                        <select id="m_status" class="form-control" name="status">
                            <option value="{{ \TCG\Voyager\Models\MenuItem::STATUS_ACTIVE }}"
                                    selected="selected">{{ __('voyager::menu_builder.status_active') }}</option>
                            <option
                                value="{{ \TCG\Voyager\Models\MenuItem::STATUS_HIDDEN }}">{{ __('voyager::menu_builder.status_hidden') }}</option>
                            <option
                                value="{{ \TCG\Voyager\Models\MenuItem::STATUS_INACTIVE }}">{{ __('voyager::menu_builder.status_inactive') }}</option>
                        </select><br>

                        <label for="page_id">{{ __('voyager::menu_builder.page_id') }}</label>
                        <select id="m_page_id" class="form-control" name="page_id">
                            <option value="0">{{ __('voyager::menu_builder.no_value') }}</option>
                            @foreach($itemsPage as $idPage=>$titlePage)
                                <option value="{{ $idPage }}">({{ $idPage }})-{{ $titlePage }}</option>
                            @endforeach
                        </select>
        ...
        $(document).ready(function () {
            ...
            $m_modal.on('show.bs.modal', function (e, data) {
                ...
                if (_adding) {
                    ...
                    // Изменение значения поля Страница для пункта меню
                    var nameField = 'page_id';
                    var valField = _src.data(nameField);
                    $m_page_id.find("option[value!='" + valField + "']").removeAttr('selected');
                    $m_page_id.find("option[value='" + valField + "']").attr('selected', 'selected');
                    $m_page_id.val(valField);
